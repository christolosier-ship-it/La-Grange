...
export function createDashboardHero(sync: SyncState | undefined): HTMLElement {
  ...
  header.append(title, createSyncButton(sync));
  if (sync?.snapshot?.syncedAt) {
    const time = document.createElement('time');
    time.dateTime = sync.snapshot.syncedAt;
    time.textContent = formatRelativeDate(sync.snapshot.syncedAt);
    time.title = formatFullDate(sync.snapshot.syncedAt);
    header.append(time);
  }
  return header;
}
...
export function createDashboardLoadingState(sync: SyncState | undefined): HTMLElement {
  ...
  message.textContent = sync?.status === 'offline'
    ...
    : 'La première carte apparaîtra dès que les données GitHub seront prêtes.';
...
export function createDashboardStats(projects: readonly Project[]): HTMLElement {
  ...
  section.style.setProperty(
    '--stats-beam',
    `url("${PHASE_6_ASSET_BASE}p6-c11-stats-beam-1600x220.webp")`,
  );
  section.append(
    createStat(statistics.total, 'Projets'),
    createStat(statistics.active, 'Actifs'),
    createStat(statistics.launchable, 'Applications'),
    createStat(statistics.archived, 'Archives'),
  );
  return section;
}
